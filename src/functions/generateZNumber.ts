...
const generateZNumber: Handler = async (_event: any, _context?: Context): Promise<APIGatewayProxyResult> => {
  const numberService = new NumberService(new DynamoDBService());

  try {
    const generatedZNumber: ZNumber = await numberService.createZNumber(1, null);
